...
const PDFViewer = () => {
  ...
  const renderTextOverlay = async (page: any, viewport: any) => {
    ...
    for (const item of textContent.items) {
      ...
      const words = tokens.map((token: string) => {
        ...
        // Half-bold only
        if (halfBoldEnabled && !accentEnabled) {
          if (len <= 2 || !isLetter) return `<span style="font-weight:bold">${escapeHtml(token)}</span>`;
  ...
  return (
    <div className="flex flex-col h-screen text-white">
      ...
      <div className="flex-1 flex justify-center p-4 w-full overflow-auto">
        <div
        ...
        >
          <canvas
            ref={canvasRef}
            className={`transition-opacity absolute top-0 left-0 duration-300 ${fade && !dyslexiaEnabled ? "opacity-100" : "opacity-0"}`}
          />

          <div
            ref={overlayRef}
            className={`absolute top-0 left-0 pointer-events-none whitespace-pre text-black bg-white ${dyslexiaEnabled ? "opacity-100" : "opacity-0"}`}
          />
        </div>
      </div>
    </div>
